...
//----------------------------------------------------------------------------------------------------------------------------------//
        ...
        ajaxPost("https://oc-jswebsrv.herokuapp.com/api/lien", lienFormulaire,
            function() {
                ...
                // Utilisation d'ajaxGet pour récupérer les données sur le serveur, et les afficher
                ajaxGet("https://oc-jswebsrv.herokuapp.com/api/liens", function(data) {
                    // Récupération des données du serveur
                    var donneesRecues = JSON.parse(data);
                    // Création de l'objet lienElt, à partir du prototype lienVide.
                    var lienElt = Object.create(lienVide);
                    // On définit les valeurs des propriétés de notre nouvel objet lienElt
                    // Ces valeurs sont bien celles récupérées du serveur
                    lienElt.titre = donneesRecues[0].titre;
                    lienElt.url = donneesRecues[0].url;
                    lienElt.auteur = donneesRecues[0].auteur;
                    lienElt.init(lienElt.titre, lienElt.url, lienElt.auteur);
                    ...
                    // Stockage du lien grâce au Web sessionStorage
                    // (Je ne vois pas comment faire autrement, même en ayant récupéré les données du serveur)
                    var monobjet_json = JSON.stringify(lienElt);
...
// Gestionnaire d'évènement au rechargement de la page
window.addEventListener("load", function(){
  // Récupération des données stockées
  var monobjet_json = sessionStorage.getItem("objet");
  var monobjet = JSON.parse(monobjet_json);
  // Création du nouveau lien et positionnement au dessus de la liste
  var nouveauLien = creerElementLien(monobjet);
  divContenuElt.insertBefore(nouveauLien, divContenuElt.firstChild);
});
